...
var pswd = process.env.PASSPHRASE_PKEY
var content = fs.readFileSync('encryptionkeys/eprivate.pem', 'utf8')
const privateKey = CryptoJS.AES.decrypt(content, pswd).toString(CryptoJS.enc.Utf8)
...
export const authorize = (user) => {
  let toSign = {...user}
  delete toSign.data.password
  return jwt.sign(toSign, privateKey, { expiresIn: '6h', algorithm: 'RS256' })
}
export const verify = (token: string) => token ? (jws.verify as ((token: string, secret: string) => boolean))(token, publicKey) : false
export const decode = (token: string) => { return jws.decode(token).payload }

export const sanitizeUserHtml = (html: string) => sanitizeHtml(html, {allowedTags: [], allowedAttributes: {}})
